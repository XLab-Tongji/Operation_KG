...
input_dir    = rootPath+'/logs/'  # The input directory of log file
output_dir   = rootPath+'/demo/IPLoM_result/'  # The output directory of parsing results
...
def loadData(infile):
    ...
    infile = rootPath+'/demo/IPLoM_result/'+infile
    infile.encode('utf-8')
    f = open(infile,'r')
...
def outputDataset(infile):
    ...
    global new_dataset
    new_dataset=[]
    outfile = rootPath+'/demo/IPLoM_result/'+'new' + infile + '.txt'
...
def checkCycle(infile):
    ...
    for i in range(0,len(new_dataset)):
        ...
        while j < len(new_dataset[i]):
            ...
            j+=1
    print(new_dataset)
    outfile = rootPath+'/demo/IPLoM_result/'+'new2' + infile + '.txt'
    outfile.encode('utf-8')
    f=open(outfile,'w')
    for i in range(0,len(new_dataset)):
        for j in range(0,len(new_dataset[i])-1):
